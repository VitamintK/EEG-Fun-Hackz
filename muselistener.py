...
#    server.shutdown()
    ...
    if len(key_buffer) == 5:
        k = key_buffer
        key_buffer = ""
        SendInput(Keyboard(low_letters[int(k, 2)]))
    if len(key_buffer) > 5:
        k = key_buffer[:5]
        key_buffer = ""
        SendInput(Keyboard(low_letters[int(k, 2)]))
...
def calibrate():
    input("don't blink. Press enter to start calibration.")
    time.sleep(1)
    print("Serving on {}".format(server.server_address))
    server.serve_forever()
    medians = map(statistics.median, zip(*ranges))
    maxes = map(max, zip(*ranges))
    mins = map(min, zip(*ranges))
    diffs = [a-b for a,b in zip(maxes, mins)]
    max_noise.extend(diffs)
    return list(medians)

def start_wink(eye):
    #print(ranges)
    print("start wink")
    input("wink once with your {} eye.".format(eye))
    time.sleep(1)
    print("go")
    server.serve_forever()
    print(ranges)
    ...
    mins = map(min, zip(*ranges))
    diffs = [a-b for a,b in zip(maxes, mins)]
    main_difference = {'value': max(diffs), 'index': diffs.index(max(diffs))}
    threshold = {'index': main_difference['index'], 'value': (max_noise[main_difference['index']]+main_difference['value'])/2}
    return threshold
...
import string
low_letters = [KEY_A,KEY_B,KEY_C,KEY_D,KEY_E,KEY_F,KEY_G,KEY_H,KEY_I,KEY_J,KEY_K,KEY_L,KEY_M,KEY_N,KEY_O,KEY_P,KEY_Q,KEY_R,KEY_S,KEY_T,KEY_U,KEY_V,KEY_W,KEY_X,KEY_Y,KEY_Z,VK_SPACE,VK_SPACE,VK_SPACE,VK_SPACE,VK_SPACE,VK_BACK]
...
if __name__ == "__main__":
    ...
    dis.map("/debug", print)
    dis.map("/muse/eeg", calibrate_base, "EEG")
    server = osc_server.ThreadingOSCUDPServer(
            (args.ip, args.port), dis)
    if(input("Calibrate? type n to not.") == 'n'):
        with open("calibrations", "rb") as f:
            p = pickle.load(f)
            max_noise, left, right = p
    else:
        normals = calibrate()
        print("done.")
        dis._map["/muse/eeg"] = []
        dis.map("/muse/eeg", calibrate_wink, "EEG")
        print("line 92")
        
        ranges = []
        print("line 96")
        left = start_wink("left")
        ranges = []
        right = start_wink("right")
        with open("calibrations", 'wb') as f:
            pickle.dump([max_noise, left, right], f)
    dis._map["/muse/eeg"] = []
    dis.map("/muse/eeg", eeg_handler, "EEG")
    server.serve_forever()
